 ...
 *
 *  ABOUT
 *  This package automates a generalized model-optimization algorithm supporting models wit six or fewer 
 *  coefficients, and then provides visualizations of the input data, best guess, optimized model, and  
 *  error of various guesses based on input search behavior.
 *
 *  REQUIREMENTS
 ...
 *  This program assumes some basic knowledge of Processing and Java, for it will be required to load user
 *  data and write the desired model equation. All functions
 ...
 *  for the optimal values, and descent parameters.
 *
 ...
 *  in Tabs 2, 3, and 4. 
 *  For Tabs 2, 3, and 4, clicking anywhere will "freeze" the rotation of the visualization; clicking again will 
 *  "unfreeze" it. Switching between tabs will always "unfreeze" the rotation. Visualized variables still may be 
 *  switched even when the rotation is frozen. 
 ...
 *  The greater the error for the set of values at that combination of variables, the more red it will be. The less 
 *  erroneous, the closer the color will be to white. 
 *  
 *
 *  FEATURE SCOPE
